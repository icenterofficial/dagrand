...
import { Romduol } from '../components/Romduol.tsx';
            ...
            {filteredArticles.map((article) => (
                <div key={article.id} className="group bg-white dark:bg-slate-900 rounded-xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-500 border border-slate-100 dark:border-slate-800 flex flex-col h-full">
                    ...
                    <div className="p-8 flex-1 flex flex-col relative">
                        ...
                        <h2 className="text-xl font-serif font-bold text-brand-900 dark:text-white mb-4 line-clamp-2 leading-tight group-hover:text-gold-600 transition-colors">{article.title}</h2>
                        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed mb-6 line-clamp-3">{article.excerpt}</p>
                        <div className="mt-auto pt-6 border-t border-slate-100 dark:border-slate-800">
                            <Link to={`/updates/${article.id}`} className="inline-flex items-center text-gold-600 font-bold uppercase tracking-widest text-xs hover:text-gold-500 transition-colors group-hover:gap-2">
