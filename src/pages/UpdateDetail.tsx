...
const UpdateDetail = () => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-brand-gray">
        <div className="text-center">
          <h2 className="text-2xl font-serif font-bold text-brand-navy mb-4">Article Not Found</h2>
          <Link 
            to="/updates" 
  ...
  const articleSchema = {
    ...
    "publisher": {
      ...
      "logo": {
        "@type": "ImageObject",
        "url": "https://dagrand.net/logo.png"
      }
    },
       ...
       <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24">
           <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
               ...
               <div className="lg:col-span-8">
                   <Section>
                       <div className="prose prose-lg prose-headings:font-serif prose-headings:text-brand-navy prose-p:text-gray-600 prose-p:font-light prose-p:leading-loose prose-a:text-brand-gold hover:prose-a:text-brand-navy max-w-none first-letter:text-5xl first-letter:font-serif first-letter:text-brand-gold first-letter:mr-3 first-letter:float-left">
                           <p className="lead font-medium text-xl text-brand-navy italic mb-8 border-l-4 border-brand-gold pl-4">
                               {update.summary}
                           </p>
                        ...
                        {update.author && (
                            <div className="mt-16 p-8 bg-brand-gray border border-gray-100 rounded-sm flex items-center gap-6">
                                ...
                                <div>
                                    ...
                                    <h3 className="font-serif font-bold text-xl text-brand-navy">{update.author.name}</h3>
                                    <p className="text-gray-500 text-sm mt-1">{update.author.role}</p>
                                </div>
                            </div>
                        )}

                       {/* Share / Tags Area */}
                       <div className="mt-8 pt-8 border-t border-gray-100 flex justify-between items-center relative">
                           ...
                           <div className="relative">
                               <button 
                                   ...
                                   className={`p-2.5 rounded-full transition-all duration-300 ${isShareOpen ? 'bg-brand-gold text-white rotate-180' : 'bg-gray-100 hover:bg-brand-navy hover:text-white text-gray-600'}`}
                               >
                               ...
                               <AnimatePresence>
                                   {isShareOpen && (
                                       <motion.div
                                           ...
                                           className="absolute bottom-full right-0 mb-3 bg-white border border-gray-200 shadow-2xl rounded-xl p-2 min-w-[180px] z-30"
                                       >
                                           <div className="flex flex-col gap-1">
                                               {shareActions.map((item, idx) => (
                                                   <button 
                                                       key={idx}
                                                       onClick={item.action}
                                                       className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-gray-600 transition-colors ${item.color} group`}
                                                   >
                                                       <span className="text-gray-400 group-hover:text-inherit transition-colors">{item.icon}</span>
                                                       {item.name}
                                                   </button>
                                               ))}
                                           </div>
                                           {/* Arrow Pointer */}
                                           <div className="absolute -bottom-1.5 right-3 w-3 h-3 bg-white border-b border-r border-gray-200 rotate-45"></div>
                                       </motion.div>
                                   )}
                               </AnimatePresence>
                           </div>
               ...
               <div className="lg:col-span-4">
                   <div className="sticky top-28 space-y-12">
                       ...
                       <div className="bg-brand-gray p-8 rounded-sm">
                           <h3 className="font-serif font-bold text-xl text-brand-navy mb-6 pb-2 border-b-2 border-brand-gold inline-block">
                               Recent Updates
                           </h3>
                           <div className="space-y-6">
                               {relatedUpdates.map((rel) => (
                                   <Link key={rel.id} to={`/updates/${rel.id}`} className="block group">
                                       ...
                                       <h4 className="font-serif font-bold text-brand-navy group-hover:text-brand-gold transition-colors line-clamp-2 leading-snug">
                                           {rel.title}
                                       </h4>
                                   </Link>
                               ))}
                           ...
                           <div className="mt-8 pt-6 border-t border-gray-200">
                               <Link to="/updates" className="text-xs font-bold uppercase tracking-widest text-brand-navy hover:text-brand-gold transition-colors">
                                   View All Posts
                               </Link>
                           </div>
                       </div>
