...
import { TEAM_INTRO, TEAM_MEMBERS } from '../../constants';
...
const Team = () => {
  return (
    <div className="bg-white min-h-screen font-sans">
      ...
      <PageHeader 
          title="Our Team" 
          subtitle="Highly skilled and experienced lawyers ensuring insightful, strategic, and globally informed legal solutions." 
      />

      {/* Intro Section - Redesigned */}
      <section className="py-20 bg-white relative overflow-hidden">
         <div className="absolute top-0 right-0 w-1/3 h-full bg-brand-gray -z-10 skew-x-12 opacity-50 origin-top"></div>
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid lg:grid-cols-12 gap-12 items-start">
               ...
               <div className="lg:col-span-5">
                  <Section>
                      <div className="relative">
                          ...
                          <h2 className="text-3xl md:text-4xl font-serif font-bold text-brand-navy leading-tight relative z-10">
                             A synergy of local depth and global perspective.
                          </h2>
                      </div>
                      <div className="mt-8 w-20 h-1 bg-brand-gold"></div>
                      <p className="mt-8 text-lg text-brand-navy font-medium italic">
                         {TEAM_INTRO[0]}
                      </p>
                  </Section>
               </div>
               ...
               {/* Right: Detailed text */}
               <div className="lg:col-span-7 space-y-8 text-gray-600 font-light text-lg leading-relaxed">
                   <Section>
                       <p>{TEAM_INTRO[1]}</p>
                   </Section>
                   <Section>
                       <div className="p-8 bg-brand-navy text-gray-300 rounded-sm relative overflow-hidden shadow-lg">
                           ...
                           <p className="relative z-10">{TEAM_INTRO[2]}</p>
                       </div>
                   </Section>
               </div>
            </div>
               ...
               <Section key={index}>
                  <div className={`flex flex-col lg:flex-row gap-0 lg:gap-16 items-stretch ${index % 2 === 1 ? 'lg:flex-row-reverse' : ''}`}>
                      ...
                      <div className="lg:w-7/12 flex flex-col justify-center lg:py-12">
                          <div className="bg-white p-8 pt-10 lg:p-12 rounded-b-sm lg:rounded-sm rounded-t-none lg:rounded-t-sm shadow-sm border border-gray-100 border-t-0 lg:border-t relative z-0">
                              {/* Desktop Header */}
                              <div className="hidden lg:block mb-8 border-b border-gray-100 pb-6">
                                  <div className="flex items-center justify-between">
                                     <div>
                                        <h3 className="text-4xl font-serif font-bold text-brand-navy mb-2">{member.name}</h3>
                                        <p className="text-brand-gold font-bold uppercase tracking-[0.2em] text-sm">{member.role}</p>
                                     </div>
                                     <User className="h-10 w-10 text-brand-gray" />
                                  </div>
                              </div>

                              {/* Languages */}
                              <div className="flex flex-wrap items-center gap-3 mb-6">
                                  <div className="flex items-center gap-2 bg-brand-gray px-3 py-1 rounded-full border border-gray-200">
                                      <Globe className="h-4 w-4 text-brand-navy" />
                                      <span className="text-xs font-bold text-brand-navy uppercase tracking-wider">Languages</span>
                                  </div>
                                  <span className="text-gray-600 text-sm font-medium">{member.languages}</span>
                              </div>

                              {/* Bio */}
                              <div className="space-y-4 text-gray-600 leading-relaxed font-light mb-8 text-lg text-justify">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-6 border-t border-gray-100">
                                  <div>
                                      <div className="flex items-center gap-2 mb-3 text-brand-navy">
                                          ...
                                          <span className="font-bold uppercase tracking-wider text-xs">Education</span>
                                      </div>
                                      <p className="text-sm text-gray-600 leading-snug">{member.education}</p>
                                  </div>
                                  ...
                                  <div className="space-y-3">
                                      <div className="flex items-center gap-2 mb-3 text-brand-navy">
                                          ...
                                          <span className="font-bold uppercase tracking-wider text-xs">Contact</span>
                                      </div>
                                      <a href={`tel:${member.contact.phone}`} className="flex items-center gap-3 text-sm text-gray-600 hover:text-brand-navy transition-colors group/link">
                                          <Phone className="h-4 w-4 text-gray-400 group-hover/link:text-brand-gold transition-colors" />
                                          {member.contact.phone}
                                      </a>
                                      <a href={`mailto:${member.contact.email}`} className="flex items-center gap-3 text-sm text-gray-600 hover:text-brand-navy transition-colors group/link">
